...
function insertUser(request, response) {
    ...
    mysqlconnection.query('insert into users values(?,?,?)', [userid, email, password], checkStatus);
...
function upadateUser(request, response) {
    ...
    mysqlconnection.query('update users set email = ? , password = ? where userid = ?', [email, password, userid], checkStatus);
    response.send(statusmsg);
}
